...
{
  if (!_store_address(NVM_REG_BASE + NVM_REG_STATUS_OFFS))
    return false;
  ...
  {
    if (!pdi_send(&cmd, 1) || !pdi_recv(&status, 1))
      break;
    if (!(status & NVM_STATUS_BUSY_bm))
      return true;
  }
...
{
  uint8_t status = 0;
  if (!_ldcs(PDI_REG_STATUS, &status))
    return false;
                    ...
                    uint16_t len)
{
  ...
  {
  case NVM_FLASH:
    cmd = NVM_ERASE_WRITE_FLASH_PAGE;
    break;
  case NVM_APPLICATION:
    cmd = NVM_ERASE_WRITE_APP_SECTION_PAGE;
    break;
  case NVM_BOOT:
    cmd = NVM_ERASE_WRITE_BOOT_SECTION_PAGE;
    break;
  case NVM_EEPROM:
    _RETRY_LOOP(_write_eeprom_page(addr, buf, len));
  case NVM_SIGNATURE:
    if (!nvm_erase_page(type, addr))
      return false;
...
bool nvm_erase_page(nvm_t type, uint32_t addr)
{
  ...
  {
  case NVM_FLASH:
    cmd = NVM_ERASE_FLASH_PAGE;
    break;
  case NVM_APPLICATION:
    cmd = NVM_ERASE_APP_SECTION_PAGE;
    break;
  case NVM_BOOT:
    cmd = NVM_ERASE_BOOT_SECTION_PAGE;
    break;
  case NVM_EEPROM:
    cmd = NVM_ERASE_EEPROM_PAGE;
    break;
  case NVM_SIGNATURE:
    cmd = NVM_ERASE_USERSIG_ROW;
    break;
  case NVM_FUSE:
  case NVM_NONE:
    break;
